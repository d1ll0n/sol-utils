...
contract UniswapV2HelperTest is Test, BasePairTest {
    ...
    function mulOverflows(
        uint256 a,
        uint256 b
    ) internal pure returns (bool overflow, uint256 c) {
        unchecked {
    ...
    function calculateAmountOut(
        ...
        (bool overflows1, uint256 amountInWithFee) = mulOverflows(
            amountIn,
            997
        );
        (bool overflows2, uint256 numerator) = mulOverflows(
            amountInWithFee,
            reserveOut
        );
        (bool overflows3, uint256 denominator1) = mulOverflows(reserveIn, 1000);
        (bool overflows4, uint256 denominator) = addOverflows(
            denominator1,
            amountInWithFee
        );
        assembly {
    ...
    function calculateAmountIn(
        ...
        (bool overflows1, uint256 numerator1) = mulOverflows(
            reserveIn,
            amountOut
        );
        (bool overflows2, uint256 numerator) = mulOverflows(numerator1, 1000);
        (bool overflows3, uint256 denominator1) = subOverflows(
            reserveOut,
            amountOut
        );
        (bool overflows4, uint256 denominator) = mulOverflows(
            denominator1,
            997
        );
        assembly {
    ...
    function test_calculateAmountOut1(
        ...
        (uint256 amountOut, bool overflows) = calculateAmountOut(
            amountIn,
            reserveIn,
            reserveOut
        );
    ...
    function test_calculateAmountOut2(
        ...
        if (overflows) {
            vm.expectRevert(stdError.arithmeticError);
            referenceHelper.calculateV2AmountOut(
                amountIn,
                reserveIn,
                reserveOut
            );
    ...
    function checkOverflowsV2AmountIn(
        ...
        internal
        pure
        returns (bool overflow0, bool overflow1, bool overflow2, bool overflow3)
    {
        assembly {
            // ((xy / y) != x) * y
    ...
    function test_calculateAmountIn1(
        ...
        (uint256 amountIn, bool overflows) = calculateAmountIn(
            amountOut,
            reserveIn,
            reserveOut
        );
    ...
    function test_calculateAmountIn2(
        ...
        if (overflows) {
            vm.expectRevert(stdError.arithmeticError);
            referenceHelper.calculateV2AmountIn(
                amountOut,
                reserveIn,
                reserveOut
            );
    ...
    function testGetAmountIn(
        ...
        address tokenOut = pairZeroForOne == swapZeroForOne
            ? pairedToken
            : token;
        amountOut = bound(amountOut, 1, tokenOut.balanceOf(pair) - 1);
        uint256 amountIn = helper.getV2AmountIn(
            pair,
            swapZeroForOne,
            amountOut
        );
        assertEq(
            referenceHelper.getV2AmountIn(pair, swapZeroForOne, amountOut),
            amountIn
        );
    }
    ...
    function testGetAmountOut(
        ...
        uint256 amountOut = helper.getV2AmountOut(
            pair,
            swapZeroForOne,
            amountIn
        );
        assertEq(
            referenceHelper.getV2AmountOut(pair, swapZeroForOne, amountIn),
            amountOut
        );
    }
    ...
    function testExecuteV2Swap(
        ...
        uint256 amountIn = referenceHelper.getV2AmountIn(
            pair,
            swapZeroForOne,
            amountOut
        );
    ...
    function testFailExecuteV2SwapOverflow(
        ...
        uint256 amountOut = helper.getV2AmountIn(
            pair,
            swapZeroForOne,
            amountIn
        );
