...
contract LogsContextTest is Test {
    ...
    function test_addWatchedEvent() external {
        ...
        function(Log memory)
            internal
            pure
            returns (string memory) fn = serializeTransfer;
        assembly {
            fnIdExpected := fn
        }
        ...
        assertEq(
            fnIdActual,
            fnIdExpected,
            "watchedEvents[0].serializeLog != serializeTransfer"
        );
        assertEq(
            context.watchedEventSignatures[0],
    ...
    function getLogsContextExpectingTransfer()
        internal
        returns (LogsContext memory)
    {
    ...
    function test_expectRecordedLogs() external {
        ...
        assertEq(
            context.expectedLogHashes.length,
            1,
            "expectedLogHashes.length != 1"
        );
        Log memory log = context.expectedLogs[0];
        assertEq(
            log.hashLog(),
            context.expectedLogHashes[0],
            "log.hashLog() != expectedLogHashes[0]"
        );
        checkTransferLog(
            log,
            address(emitter),
