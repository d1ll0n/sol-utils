...
contract ArrayHelpersTest is Test, MemoryPointerAssertions {
    ...
    function test_cloneArray() external {
        ...
        assertBufferEq(
            ...
            newArray,
            0x80,
            "should copy the array length and element heads"
        );
    }
    ...
    function test_cloneAndPush() external {
        ...
        assertEq(
            newArray.readUint256(),
            4,
            "should write the new array length"
        );
        assertBufferEq(
            oldArray.next(),
            newArray.next(),
        ...
        assertEq(
            newArray.offset(0x80).readUint256(),
            4,
            "should write the new element"
        );
    }
    ...
    function test_map() external {
        ...
        assertEq(
            mappedArray.offset(0x40).readUint256(),
            4,
            "arr[1] should be 4"
        );
        assertEq(
            mappedArray.offset(0x60).readUint256(),
            6,
            "arr[2] should be 6"
        );
    }
    ...
    function test_filterMap() external {
        ...
        assertEq(
            filteredArray.offset(0x40).readUint256(),
            4,
            "arr[1] should be 4"
        );
    }

    function filterMapCallback(
    ...
    function test_mapWithIndexAndArg() external {
        MemoryPointer array = getUint256Array();
        MemoryPointer freePtr = getFreeMemoryPointer();
        MemoryPointer mappedArray = array.mapWithIndexAndArg(
            mapWithIndexAndArgCallback,
            2
        );
        assertEq(
            getFreeMemoryPointer(),
    ...
    function test_reduceWithArg() external {
        ...
        uint256 accumulator = array.reduceWithArg(
            reduceWithArgCallback,
            0,
            cache
        );
        assertEq(getFreeMemoryPointer(), freePtr, "should not allocate memory");
        ...
        assertEq(
            cache.readUint256(),
            3,
            "should write previous accumulator to cache"
        );
    }
    ...
    function test_forEach() external {
        ...
        assertBufferEq(
            storedArray.toPointer(),
            array,
            0x80,
            "storage array does not match array"
        );
    }
    ...
    function findIndexWithArgPredicate(
        uint256 _element,
        uint256 _min
    ) internal pure returns (bool) {
        return _element >= _min;
    ...
    function findIndexFromPredicate(
        MemoryPointer _element
    ) internal pure returns (bool) {
    ...
    function test_findIndexFromWithArg() external {
        ...
        int256 i = array.findIndexFromWithArg(
            findIndexFromWithArgPredicate,
            1,
            arg
        );
        assertEq(getFreeMemoryPointer(), freePtr, "should not allocate memory");
    ...
    function test_countFrom() external {
        ...
        assertEq(
            array.countFrom(countFromPredicate, 1),
            2,
            "should count 2 elements"
        );
        assertEq(
            array.countFrom(countFromPredicate, 2),
            1,
