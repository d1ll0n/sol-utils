...
library EnumerableSetLib {
    ...
    function length(
        Bytes32Set storage set
    ) internal view returns (uint256 _length) {
        assembly {
    ...
    function asBytes32Set(
        AddressSet storage set
    ) internal pure returns (Bytes32Set storage _set) {
        assembly {
            _set.slot := set.slot
        }
    }
    ...
    function asUintArray(
        bytes32[] memory bValues
    ) internal pure returns (uint256[] memory aValues) {
        assembly {
            aValues := bValues
        }
    }
