...
library UniswapV2Helper {
    ...
    function calculateV2AmountIn(
        ...
        assembly {
            ...
            numeratorPart1, errorBuffer := smMul(
                reserveIn,
                amountOut,
                errorBuffer
            )
            ...
            denominatorPart1, errorBuffer := smSub(
                reserveOut,
                amountOut,
                errorBuffer
            )
            denominator, errorBuffer := smMul(
                denominatorPart1,
                997,
                errorBuffer
            )
    ...
    function getV2Reserves(
        ...
        assembly {
            mstore(
                UniswapV2_GetReserves_Selector_Pointer,
                UniswapV2_GetReserves_Selector
            )
    ...
    function getV2AmountOut(
        ...
        (uint256 reserveIn, uint256 reserveOut) = getV2Reserves(
            pair,
            zeroForOne
        );
    ...
    function executeV2Swap(
        ...
        assembly {
            ...
            mstore(
                add(freeMemoryPointer, UniswapV2_Swap_Selector_Offset),
                UniswapV2_Swap_Selector
            )
            mstore(
                add(
                    add(freeMemoryPointer, UniswapV2_Swap_Amount0Out_Offset),
                    amountOutOffset
                ),
                amountOut
            )
            ...
            mstore(
                add(freeMemoryPointer, UniswapV2_Swap_Recipient_Offset),
                address()
            )
            mstore(
