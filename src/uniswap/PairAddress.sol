...
library PairAddress {
    ...
    function calculateBaseSwapPair(
        ...
        return
            calculateCreate2Address(
                BaseSwap_Create2Prefix,
                pairSalt,
                BaseSwap_InitCodeHash
            );
        // assembly {
        //     let ptr := mload(0x40)
        //     // Write 0xff + address to bytes 11:32
    ...
    function calculateQuickSwapPair(
        address token0,
        address token1
        ...
        return
            calculateCreate2Address(
                QuickSwap_Create2Prefix,
                pairSalt,
                QuickSwap_InitCodeHash
            );
    }

    function getQuickSwapPair(
        address tokenA,
        address tokenB
    ) internal pure returns (address pair) {
        (address token0, address token1) = tokenA < tokenB
